...
public:
	...
	0  0 -1  1  1  0  kfmrh
	*/
	void  mvfs(QVector3D coordinate);//构造一个体 一个面 一个外环 一个点
	Vertex* mev(Vertex *sv, Loop* lp, QVector3D coordinate);//生成一个新点 同时构造一条连接新点与另一给定点的边（半边）
	...
	void semv(Edge* e, QVector3D coordinate);//给定边上构造一个新点，同时将边分离为两条边
	void sweep(QVector3D oritation, Loop *loop);//扫成操作
	void kemr(Vertex*sv, Vertex* ev, Loop *loop);//消去环中一条边 构造一个内环
	void semv(Vertex*sv, Vertex* ev, QVector3D coordinate);//给定边上构造一个新点，同时将边分离为两条边
